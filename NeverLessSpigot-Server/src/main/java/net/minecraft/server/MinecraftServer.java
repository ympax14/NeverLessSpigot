...
import me.ympax.neverlessspigot.tickloop.ReentrantIAsyncHandler;
import me.ympax.neverlessspigot.tickloop.TasksPerTick;
import me.ympax.neverlessspigot.world.WorldTickManager;
// NeverLessSpigot end
...
public abstract class MinecraftServer extends ReentrantIAsyncHandler<TasksPerTick> implements ICommandListener, IAsyncTaskHandler, IMojangStatistics {
	...
	public java.util.Queue<Runnable> priorityProcessQueue = new java.util.concurrent.ConcurrentLinkedQueue<Runnable>(); // NeverLessSpigot
	public int autosavePeriod;
	// CraftBukkit end

	// NeverLessSpigot - instance
	...
	public MinecraftServer(OptionSet options, Proxy proxy, File file1, Thread thread) {
		super("Server"); // NeverLessSpigot - backport modern tick loop
	...
	// NeverLessSpigot - thread affinity
	...
	public void run() {
		...
		try {
			...
			if (this.init()) {
				//NeverLessSpigot - statistics
				disableStatistics = true;
				// NeverLessSpigot start - implement thread affinity
				if (NeverLessSpigotConfig.threadAffinity) {
					LOGGER.info(" ");
					...
					NeverLessSpigot.debug(AffinityLock.dumpLocks());

				}
				// NeverLessSpigot end

				// NeverLessSpigot - parallel worlds
				...
				while (this.isRunning) {
                    ...
                    if (i > 5000L && this.nextTickTime - this.lastOverloadWarning >= 30000L && ticks > 500) { // CraftBukkit // NeverLessSpigot - prevent display of overload on first 500 ticks
                        long j = i / 50L;
			...
			// NeverLessSpigot start - thread affinity
			if (lock != null) {
				lock.release();
				MinecraftServer.LOGGER.info("Released CPU " + lock.cpuId() + " from server usage.");
			}
			// NeverLessSpigot end
			// NeverLessSpigot start - stop statistics connection
			Thread statisticsThread = null;
			if (disableStatistics) {
				StatisticsClient client = this.getNeverLessSpigot().getClient();
			...
			// NeverLessSpigot - wait for statistics to finish stopping
			try {
				if (this.getNeverLessSpigot().getClient().isConnected) {
...
//                }
	...
	// NeverLessSpigot - backport modern tick loop
...
//        this.methodProfiler.b();
	...
	public void B() {
		...
		// NeverLessSpigot - move to WorldTickManager
...
//		SpigotTimings.bukkitSchedulerTimer.stopTiming(); // Spigot
		...
		// NeverLessSpigot start - priority process queue
	...
	public String getServerModName() {
		return NeverLessSpigotConfig.serverBrandName; // [Nacho-0035] // NachoSpigot - NachoSpigot > // TacoSpigot - TacoSpigot //
	...
	// NeverLessSpigot start - backport modern tick loo
	...
	// NeverLessSpigot - instance
	public NeverLessSpigot getNeverLessSpigot() {
		return this.neverlessSpigot;
	}
	
	// NeverLessSpigot - MSPT (milliseconds per tick)
