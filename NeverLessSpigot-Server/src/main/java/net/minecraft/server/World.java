...
// NeverLessSpigot start 
import me.ympax.neverlessspigot.config.NeverLessSpigotConfig;
import me.ympax.neverlessspigot.entity.EntityTickLimiter;
...
public abstract class World implements IBlockAccess {
	...
	public final List<Entity> entityList = new ObjectArrayList<Entity>() { // NeverLessSpigot - ArrayList -> ObjectArrayList 
		@Override
	...
	public final List<EntityHuman> players = new ObjectArrayList<>(); // NeverLessSpigot - ArrayList -> ObjectArrayList 
	public final List<Entity> k = new ObjectArrayList<>(); // NeverLessSpigot - ArrayList -> ObjectArrayList 
	...
	protected boolean lastTickOverload = false; // NeverLessSpigot
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
		...
		timings = new co.aikar.timings.WorldTimingsHandler(this); // Spigot - code below can generate new world and
																	// access timings
		// NeverLessSpigot - re-implement Spigot's entity max tick time, but only for certain entities
		this.entityLimiter = new me.ympax.neverlessspigot.entity.EntityTickLimiter(NeverLessSpigotConfig.entityMaxTickTime); //new org.spigotmc.TickLimiter(spigotConfig.entityMaxTickTime);
		this.tileLimiter = new org.spigotmc.TickLimiter(NeverLessSpigotConfig.tileMaxTickTime);
	}
	...
	public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
		...
		if (!flag && !this.isChunkLoaded(i, j, true)) {
			...
			// NeverLessSpigot start - configurable entity hit delay
			if (entity instanceof EntityLiving) {
				((EntityLiving) entity).maxNoDamageTicks = NeverLessSpigotConfig.hitDelay;
			}
			// NeverLessSpigot end
			
			// NeverLessSpigot start - configurable potion speeds
			if (entity instanceof EntityPotion) {
				// Subtract the speed from the y value of the velocity, the potion will drop faster and will therefore splash faster
				entity.motY -= NeverLessSpigotConfig.potionSpeed;	
				
				// Mark velocity as changed only if potion speed is not default
				if (!(NeverLessSpigotConfig.potionSpeed == 0)) {
...
//        if (!guardEntityList) { // Spigot - It will get removed after the tick if we are ticking // Paper - moved down
	...
	public void tickEntities() {
		boolean tickOverload = false; // NeverLessSpigot
		...
		entityLimiter.initTick(); // NeverLessSpigot - re-implement Spigot's entity max tick time, but only for certain entities
		for (tickPosition = 0; tickPosition < entityList.size(); tickPosition++) {
			...
			// NeverLessSpigot - synchronize
			synchronized (entity) {
				// NeverLessSpigot start - re-implement Spigot's entity max tick time, but only for certain entities
...
//            this.tileEntityList.removeAll(this.c);//c  == tileEntityListUnload
		...
		lastTickOverload = tickOverload; // NeverLessSpigot
	}
	...
	public float a(Vec3D vec3d, AxisAlignedBB axisalignedbb) {
		...
		// NeverLessSpigot start - switch to mathhelper instead of Java math
