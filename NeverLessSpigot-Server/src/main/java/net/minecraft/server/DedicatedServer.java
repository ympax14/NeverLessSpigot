...
import me.ympax.neverlessspigot.commons.IPUtils;
...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
    ...
    public DedicatedServer(joptsimple.OptionSet options, Thread thread1) { // NeverLessSpigot - backport modern tick loop
        super(options, Proxy.NO_PROXY, DedicatedServer.a, thread1);
		// CraftBukkit end
		if (!NeverLessSpigotConfig.disableInfiniSleeperThreadUsage) {
	...
	protected boolean init() throws IOException {
		...
		if (!this.p.a() && !eulaAgreed) { // Spigot
			...
			NeverLessSpigotConfig.init((File) options.valueOf("neverlessspigot-settings"));
			// NeverLessSpigot end
			...
			this.neverlessSpigot = new NeverLessSpigot(); // NeverLessSpigot
			...
			if (!this.getOnlineMode()) {
				...
				if (org.spigotmc.SpigotConfig.bungee) {
					...
					if (!NeverLessSpigotConfig.stopNotifyBungee) {
						DedicatedServer.LOGGER
								.warn("---------------------------- NeverLessSpigot Checker ----------------------------");
						DedicatedServer.LOGGER.warn(
								"If you don't want to see this message anymore, set \"settings.stop-notify-bungee\" to \"true\" in \"NeverLessSpigot.yml\"!");
