...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
	...
	// NeverLessSpigot - queue-able packets
	...
	public void c() {
		...
		if (NeverLessSpigotConfig.modernKeepalive) {
		...
		if (this.b(packetplayinflying)) {
			...
			if (!this.player.viewingCredits) {
				...
				if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
					...
					if (NeverLessSpigotConfig.firePlayerMoveEvent && PlayerMoveEvent.getHandlerList().getRegisteredListeners().length != 0) { // NeverLessSpigot - only handle player move event when there are listeners // Spigot - don't skip any move events
						...
						if (event.isCancelled()) {
		                    NeverLessSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho
                    ...
                    NeverLessSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho - register movement
				}
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (packetplayinblockplace.getFace() == 255) {
			...
			if (!throttled) {
				...
				// SportPaper end
			} else if (MinecraftServer.currentTick - lastDropTick > 1 && NeverLessSpigotConfig.fixEatWhileRunning) {
				this.player.playerInteractManager.useItem(this.player, this.player.world, itemstack);
			}
		...
		if (itemstack == null || itemstack.l() == 0) {
			...
			// KigPaper-0172
			if (NeverLessSpigotConfig.stopDecodingItemStackOnPlace
	...
	public void a(PacketPlayInUseEntity packetplayinuseentity) {
		...
		if (entity != null) {
			...
			if (!flag) {
                ...
                d0 = (NeverLessSpigotConfig.improvedHitDetection) ? 12.75D : 9.0D;
            } else {
            	// NeverLessSpigot start - configurable reach cap            	
            	if (NeverLessSpigotConfig.creativeBypass && this.player.playerInteractManager.getGameMode() == EnumGamemode.CREATIVE) {
            		d0 = 36.0;
            	} else {
                	d0 = NeverLessSpigotConfig.maxReachSqrd;
            	}	
            }
	...
	public void a(PacketPlayInWindowClick packetplayinwindowclick) {
		...
		if (this.player.activeContainer.windowId == packetplayinwindowclick.a()
			...
			// NeverLessSpigot - remove dead code
...
//        }
		...
		} else if (NeverLessSpigotConfig.kickOnIllegalBehavior) {
	...
	// NeverLessSpigot start - queue-able packets
