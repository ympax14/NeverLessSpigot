...
public abstract class EntityLiving extends Entity {
	...
	public EntityLiving(World world) {
		...
		this.k = true;
		// NeverLessSpigot start - use faster randoms
	...
	public void a(double x, double z, DamageSource source) {
		if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
			...
			if (source instanceof EntityDamageSourceIndirect) {
				EntityDamageSourceIndirect damageSourceIndirect = (EntityDamageSourceIndirect) source;
				Entity entity = damageSourceIndirect.i();
			...
			// NeverLessSpigot start - knockback addition config
			...
			if (kb.getLimitVertical() && this.motY > kb.getVerticalMax()) {
				this.motY = kb.getVerticalMax();
			}

			if (this instanceof EntityPlayer && isProjectile) {
				EntityPlayer victim = (EntityPlayer)this;

				final Vector velocity = new Vector(this.motX, this.motY, this.motZ);
				...
				if (!event.isCancelled()) {
					if (!velocity.equals(event.getVelocity())) {
							((Player) victim.getBukkitEntity()).setVelocity(velocity);
					}

					victim.playerConnection.sendPacket(new PacketPlayOutEntityVelocity(victim.getId(), this.motX, this.motY, this.motZ));
					victim.motX = this.motX;
					victim.motY = this.motY;
					victim.motZ = this.motZ;
				}
			}
		}
	}
	...
	/*public void a(double x, double z, DamageSource source) {
		if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            this.motY /= frictionVertical;
            this.motZ /= frictionHorizontal;
            this.motX -= x / (double) f1 * (double) f2;
            this.motY += (double) f3;
            this.motZ -= z / (double) f1 * (double) f2;
	...
	public void m() {
		...
		// NeverLessSpigot start - smoother mob AI disable
	...
	public boolean hasLineOfSightAccurate(Entity entity) {
		...
		if (entity instanceof EntityPlayer && NeverLessSpigotConfig.improvedHitDetection) {
