...
public abstract class EntityHuman extends EntityLiving {
	...
	protected void b(ItemStack itemstack, int i) {
		...
		if (itemstack.m() == EnumAnimation.EAT) {
			for (int j = 0; j < i; ++j) {
				// NeverLessSpigot - use faster randoms
	 ...
	 * if (!entity.l(this)) {
	 * float f = (float)
	 * this.getAttributeInstance(GenericAttributes.ATTACK_DAMAGE).getValue();
	 * float f1 = EnchantmentManager.a(this.bA(), EnumMonsterType.UNDEFINED);
	 * 
	 * if (entity instanceof EntityLiving) {
	 * f1 = EnchantmentManager.a(this.bA(), ((EntityLiving)
	 ...
	 * 
	 * if (f > 0.0F || f1 > 0.0F) {
	 * boolean flag = !world.paperSpigotConfig.disablePlayerCrits &&
	 ...
	 * EntityCombustByEntityEvent(this.getBukkitEntity(),
	 ...
	 * 
	 * if (flag2) {
	 * KnockbackProfile knockback = (entity.getKnockbackProfile() == null)
	 ...
	 * }
	 * if ((enchLvl =
	 * EnchantmentManager.getEnchantmentLevel(Enchantment.KNOCKBACK.id,
	 ...
	 * if (this.isSprinting() && knockback.isStopSprint()) {
	 * this.setSprinting(false);
	 * }
	 * if (knockback.getComboMode()) {
	 * yOff = entity.locY - this.locY;
	 * if (yOff > knockback.getComboHeight()) {
	 * this.ticksDown = MinecraftServer.currentTick;
	 * }
	 * if (yOff > knockback.getComboHeight()
	 * || MinecraftServer.currentTick - this.ticksDown < knockback.getComboTicks())
	 * {
	 ...
	 * }
	 * 
	 * // NeverLessSpigot start - more configurable knockback
	 * /*
	 * if (this.isExtraKnockback()) {
	 * entity.g(
	 * (-MathHelper.sin((float) (this.yaw * Math.PI / 180.0D)) * i
	 * profile.getWTapExtraHorizontal()),
	 * profile.getWTapExtraVertical(), (MathHelper.cos((float) (this.yaw * Math.PI /
	 * 180.0D))
	 ...
	 * profile.getExtraVertical(), (MathHelper.cos((float) (this.yaw * Math.PI /
	 * 180.0D))
	 ...
	 * }
	 * 
	 * this.applyExhaustion(world.spigotConfig.combatExhaustion); // Spigot - Change
	 * to use
	 * // configurable value
	 * } else if (flag1) {
	 * entity.extinguish();
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (flag2) {
						...
						if (knockback.getComboMode()) {
							...
							if (yOff > knockback.getComboHeight() || MinecraftServer.currentTick
									- this.ticksDown < (Integer) knockback.getComboTicks()) {
								velY = knockback.getComboVelocity();
							}
						}
						yOff = entity.locY - this.locY;
						...
						Object object = entity;
						if (entity instanceof EntityComplexPart
								&& (icomplex = ((EntityComplexPart) entity).owner) instanceof EntityLiving) {
							object = icomplex;
						}
						if (itemstack != null && object instanceof EntityLiving) {
							itemstack.a((EntityLiving) object, this);
