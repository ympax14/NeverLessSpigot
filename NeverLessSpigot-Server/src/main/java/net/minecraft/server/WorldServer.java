...
import me.ympax.neverlessspigot.async.entitytracker.MultithreadedEntityTracker;
import me.ympax.neverlessspigot.config.NeverLessSpigotConfig;
import me.ympax.neverlessspigot.world.WorldTicker;
...
public class WorldServer extends World implements IAsyncTaskHandler {
	...
	public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
		...
		// NeverLessSpigot - async entity tracking
		if (NeverLessSpigotConfig.disableTracking) {
			this.tracker = new EntityTracker(this);
		} else if (NeverLessSpigotConfig.fullAsyncTracking) {
	...
	public void doTick() {
		...
		if (this.getGameRules().getBoolean("doMobSpawning")
			...
			// NeverLessSpigot start - disable mob spawning based on tps and server load
			if (!(NeverLessSpigotConfig.limitedMobSpawns
					&& MinecraftServer.getServer().recentTps[0] < NeverLessSpigotConfig.limitedMobSpawnsThreshold) // check if tps is low 
					&& !(NeverLessSpigotConfig.stopMobSpawnsDuringOverload && lastTickOverload)) { // check if server is overloaded
				// NeverLessSpigot end
