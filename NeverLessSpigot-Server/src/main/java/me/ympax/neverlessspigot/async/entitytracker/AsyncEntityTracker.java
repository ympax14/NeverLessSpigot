package me.ympax.neverlessspigot.async.entitytracker;
...
public class AsyncEntityTracker extends EntityTracker {
	...
	private static final ExecutorService trackingThreadExecutor = Executors.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("NeverLessSpigot Entity Tracker Thread").build());
	...
	public void updatePlayers() {	
		...
		for (int i = 1; i <= NeverLessSpigotConfig.trackingThreads; i++) {
			final int finalOffset = offset++;
			
			AsyncUtil.run(() -> {
				for (int index = finalOffset; index < c.size(); index += NeverLessSpigotConfig.trackingThreads) {
