package me.ympax.neverlessspigot.config;
...
import me.ympax.neverlessspigot.async.pathsearch.AsyncNavigation;
import me.ympax.neverlessspigot.entity.EntityTickLimiter;

@SuppressWarnings("unused")
public class NeverLessSpigotConfig {

	private static final Logger LOGGER = LogManager.getLogger(NeverLessSpigotConfig.class);
	public static File CONFIG_FILE;
	protected static final YamlCommenter c = new YamlCommenter();
	private static final String HEADER = "This is the main configuration file for NeverLessSpigot.\n"
	...
	public static void init(File configFile) {
		...
		try {
			NeverLessSpigot.LOGGER.info("Loading NeverLessSpigot config from " + configFile.getName());
			config.load(CONFIG_FILE);
		} catch (IOException ignored) {
		} catch (InvalidConfigurationException ex) {
			LOGGER.log(Level.ERROR, "Could not load NeverLessSpigot.yml, please correct your syntax errors", ex);
		...
		c.setHeader(HEADER);
		readConfig(NeverLessSpigotConfig.class, null);
		...
		try {
			...
			LOGGER.warn("Please regenerate your NeverLessSpigot.yml file to prevent this issue! The server will run with the default config for now.");
	...
	static void loadComments() {
		...
		c.addComment("settings.tickless", "Configuration for tickless things.");
		...
		c.addComment("settings.command", "Configuration for NeverLessSpigot's commands");
		...
		c.addComment("settings.mob-ai-enabled", "Enables Mob AIs.");
		c.addComment("settings.command.mob-ai", "Enables the command \"/mobai\" which toggles mob ai. Users require the permission neverlessspigot.command.mobai");
	...
	private static void mobAi() {
		mobAi = getBoolean("settings.mob-ai-enabled", true);
	}
	...
	private static void serverBrandName() {
		serverBrandName = getString("settings.brand-name", "NeverLessSpigot");
	}
