...
public class NachoConfig {
	...
	// If the config has been migrated to NeverLessSpigot.yml
	...
	public static void init(File configFile) {
		...
		try {
			NeverLessSpigot.LOGGER.info("Loading NachoSpigot config from " + configFile.getName());
	...
	// Migrates all read data into the NeverLessSpigot config
	// TODO: save this data to file instead of just putting it in memory
	public static void moveToNeverLessSpigotConfig(Class<?> clazz, Class<?> NeverLessSpigotConfigClazz) {
		for (Field oldField : clazz.getDeclaredFields()) {
			...
			if (Modifier.isPublic(oldField.getModifiers()) && Modifier.isStatic(oldField.getModifiers()) && !oldField.getName().equals("CONFIG_FILE") && !oldField.getName().equals("hasMigrated")) {
				...
				for (Field newField : NeverLessSpigotConfigClazz.getDeclaredFields()) {
				...
				if (!found) {
					NeverLessSpigot.LOGGER.warn("Unable to migrate option " + oldField.getName() + "!");
				}
			}
 		}
		hasMigrated = true;
