...
public class TicksPerSecondCommand extends Command {
	...
	public boolean execute(CommandSender sender, String currentAlias, String[] args) {
		...
		if (NeverLessSpigot.getInstance().getPlayerPositionThread() != null && NeverLessSpigot.getInstance().getPlayerPositionThread().isRunning()) {
			message += ChatColor.DARK_PURPLE + "\n├ " + ChatColor.GOLD + "PlayerPosition Thread\n";
			String tpsResult = "";
			...
			if (NeverLessSpigotConfig.ticklessCombat == true) {
				...
				double[] playerPosTps = NeverLessSpigot.getInstance().getPlayerPositionThread().getTPS();
				String[] tpsAvgPlayerPosTps = new String[playerPosTps.length];

				tpsResult = org.apache.commons.lang.StringUtils.join(tpsAvgPlayerPosTps, ", ");
			}
			
			message += ChatColor.DARK_PURPLE + "├─ TPS from last 1m, 5m, 15m: " + tpsResult + "\n";
			message += ChatColor.DARK_PURPLE + "├─ Milliseconds to Run Last Tick: " + ChatColor.LIGHT_PURPLE + Math.round(NeverLessSpigot.getInstance().getPlayerPositionThread().getLastMspt() * 100.0) / 100.0;
		}
